...
The third: overruns (`TestCaseResult::Overrun`) were incorrectly mapped to `Status::Invalid` and distinguished only by a boolean flag. They now map to `Status::EarlyStop`, matching Hypothesis's `Status.OVERRUN` ordering (`OVERRUN < INVALID < VALID < INTERESTING`). Overruns count toward the generation budget (alongside invalids) but not toward `FilterTooMuch`, matching Hypothesis's separate `invalid_examples` / `overrun_examples` counters (named `invalid_test_cases` / `overrun_test_cases` in Hegel).
