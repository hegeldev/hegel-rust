...
fn draw_pad(
    ...
    let n = ntc.draw_integer::<i128>(0, 10)?;
    for _ in 0..n {
    ...
    match op {
        OpCode::Literal(cp) => {
            ...
            if state.flags & SRE_FLAG_IGNORECASE != 0 {
                ...
                if sw != c {
                    let which = ntc.draw_integer::<i128>(0, 1)?;
        ...
        OpCode::Branch(items) => {
            let idx = ntc.draw_integer::<i128>(0, items.len() as i128 - 1)? as usize;
...
fn draw_any_char(
    ...
    match alphabet {
        None => {
            let cp = ntc.draw_integer::<i128>(32, 126)?;
        ...
        Some(intervals) => {
            ...
            let idx = ntc.draw_integer::<i128>(0, n as i128 - 1)?;
            let cp = intervals
                .get(idx as isize)
...
fn emit_from_chars(
    ...
    let idx = if n > 256 && ntc.weighted(0.8, None)? {
        ntc.draw_integer::<i128>(0, 255)? as usize
    } else if n > 256 {
        ntc.draw_integer::<i128>(256, n - 1)? as usize
    } else {
        ntc.draw_integer::<i128>(0, n - 1)? as usize
    };
    out.push(chars[idx]);
    Ok(())
}
