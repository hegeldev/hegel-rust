...
/// Map a codepoint to its shrink-key position. Used by `StringChoice`'s
/// sort_key + index helpers. Mirrors Hypothesis's `IntervalSet.char_in_shrink_order`
/// for the default text alphabet: `'0'..='Z'` map to 0..=42 (digits first,
/// then uppercase), codepoints below `'0'` map to 43..=90 reversed (NUL last),
/// and everything else maps to itself.
fn codepoint_key(c: u32) -> u32 {
    if c <= 47 {
        90 - c
    } else if c <= 90 {
        c - 48
...
impl ChoiceKind {
    ...
    /// Cardinality of this kind's choice space, capped at
    /// [`crate::native::datatree::MAX_CHILDREN_EFFECTIVELY_INFINITE`].
    /// Port of upstream's `compute_max_children`.
    pub fn max_children(&self) -> crate::native::bignum::BigUint {
        use crate::native::bignum::BigUint;
        use crate::native::datatree::MAX_CHILDREN_EFFECTIVELY_INFINITE;
        use crate::native::floats::count_between_floats;

        // Count of distinct sequences over an alphabet of `alphabet_size`
        // with length in `[min_size, max_size]`, capped.
        ...
        match self {
            ...
            ChoiceKind::Float(fc) => {
                BigUint::from(count_between_floats(fc.min_value, fc.max_value))
            }
            ChoiceKind::Bytes(bc) => count_distinct_strings(256, bc.min_size, bc.max_size),
            ChoiceKind::String(sc) => {
                count_distinct_strings(sc.alpha_size(), sc.min_size, sc.max_size)
            }
        }
    }

    /// Random value sampled from this kind's domain (with kind-appropriate
    /// bias). Returns `None` for kinds without a built-in random sampler
    /// (currently string and float).
    pub fn random_value(&self, rng: &mut rand::rngs::SmallRng) -> Option<ChoiceValue> {
        use rand::RngExt;
        match self {
    ...
    /// Returns `None` for kinds without finite enumeration (currently float
    /// and string) or when the count exceeds `cap`.
...
impl From<FloatChoice> for ChoiceKind {
    fn from(c: FloatChoice) -> Self {
        ChoiceKind::Float(c)
    }
}
impl From<BytesChoice> for ChoiceKind {
    fn from(c: BytesChoice) -> Self {
        ChoiceKind::Bytes(c)
    }
}
impl From<StringChoice> for ChoiceKind {
    fn from(c: StringChoice) -> Self {
        ChoiceKind::String(c)
    }
}
