...
impl<'a> Shrinker<'a> {
    ...
    pub(crate) fn lower_duplicated_characters(&mut self) {
        ...
        for i in 0..len {
            for j in (i + 1)..(i + 1 + 4).min(len) {
                ...
                let (kind_i, val_i) =
                    match (self.current_nodes[i].kind.as_ref(), &self.current_nodes[i].value) {
                        (ChoiceKind::String(k), ChoiceValue::String(v)) => (k.clone(), v.clone()),
                        _ => continue,
                    };
                let (kind_j, val_j) =
                    match (self.current_nodes[j].kind.as_ref(), &self.current_nodes[j].value) {
                        (ChoiceKind::String(k), ChoiceValue::String(v)) => (k.clone(), v.clone()),
                        _ => continue,
                    };
    ...
    pub(crate) fn normalize_unicode_chars(&mut self) {
        let mut i = 0;
        while i < self.current_nodes.len() {
            let (kind, value) = match (self.current_nodes[i].kind.as_ref(), &self.current_nodes[i].value) {
                (ChoiceKind::String(k), ChoiceValue::String(v)) => (k.clone(), v.clone()),
                _ => {
                    i += 1;
                    continue;
