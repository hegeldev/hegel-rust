...
pass (and with it the Flaky check that would have detected the nondeterminism), end-of-run
database reconciliation, per-failure blobs, data-tree recording and its
...
the stashed payload, with no final replay, no blob, and no database entry. Multiple distinct bugs
...
deterministic foundation available, and the timeline pool was built on it.
...
- **map-database-blobs.md**: every persistence feature assumes a stored sequence
  deterministically reproduces. `Reuse` deletes an entry that replays non-interesting, with no
  retry and no report, so a p = 0.1 corpus is erased on the first lucky replay. The
  unknown-prefix-byte behaviour became the version gate for the v2 blob.
- **map-frontend-lifecycle.md**: all capture decisions precede the body, RunError variants
  flatten to one opaque string over the ABI, and Settings has no flakiness or replay-count
  knobs. This map fed the lifecycle critique's case against "no stamping needed".
- **map-shrinker.md**: the shrinker assumes the outcome is a pure function of the sequence at
  four load-bearing layers. Misalignment (the test drew differently than proposed) is well
  handled, but "the same sequence drew differently across runs" has no representation.
- **map-test-runner.md**: the three re-execution sites and their inconsistencies. The tree
  ...
  probes. The enumerated disable list above is from here, and the sketch's mode lifecycle
  carves capabilities back out of that blanket disable.
...
p = 0.1) = 0.65, so the caveated fallback fires roughly 35% of the time, making the degraded
path the common one, and 95% coverage at p = 0.1 needs about 29 replays. "No stamping needed" is
...
with `error` keeping today's aborts for exactly those users), and the caveated fallback report
survived as decision 3.

### Representation

The verdict was that draw-time validation is far too weak to anchor branch points (booleans
validate unconditionally, integers only need containment), that punning is a deliberate
shrinking mechanism widening would break, and that for the #1 workload the mechanism serves
nothing. Concurrent-stateful divergence lives inside clone streams, where the codebase's own
precedent (`clone_subtree_disabled`) already concedes per-position prediction is hopeless, so ND nodes
...
extend = 0 replay biases every ND replay towards overrun, undercounting failure probability.
...
conclusions-as-distributions because they had no consumer. Widening was scoped to
verbatim-replay contexts with punning kept for shrinker candidates, ND suffixes became values-only, and the persisted ND
bit moved to first-failure time, which became the v2-entry channel.

### Shrink loop

The verdict was that area 5 is unimplementable without three prior decisions the sketch leaves
open: where branch structure lives relative to the flat `Vec<ChoiceNode>` everything operates on,
what replaces the tree cache (which makes every proposed re-sampling a silent no-op, and whose removal
...
Beyond the gauntlet itself, the critique yielded the rule that rejected candidates stay
retryable with evidence accumulating (DRM's addition to decision 7), persistence gated on
validated acceptance, the replacement of loop-until-K-dry by generalising the existing
stochastic-pass retry machinery rather than adding a new outer loop, and a prototype order
putting cache semantics and a flat-timeline shrink experiment before any ND-node work. The flat
shrinker with a separate engine-side pool is what shipped, and the shrinker never grew
branch-aware state.

### Statistics

The verdict was that the ratchet/boost/checkpoint architecture is directionally workable but its
calibration contradicts the 10% target at three independent points, and that the "deterministic =
...
essentially all true reductions are rejected, producing shrink paralysis. Rollback against the last
checkpoint lets p decay geometrically (halvings compound across 150-plus checkpoints while every
individual checkpoint reads fine), and a tight threshold instead thrashes, with about eight
spurious rollbacks per shrink. Every p-statistic's population is undefined: per-origin versus
per-test, pooled candidate versus realized timeline. The Optimiser reuse is contradicted three ways by
...
(origin, realized timeline). Its first improvement was methodological: to build a pure-simulation
...
|---|---|---|
| Merged ND-node artifact: branch points with tree-shaped per-value suffixes in the choice sequence | Flat incumbent plus a bounded per-origin timeline pool, `POOL_CAP = 10` | Representation critique: no stable trunk under shrinking, and no comparison site ever sees an ND node (decision 5) |
| Misfit-anchored widening at draw-time constraint violations | Structural divergence signals, and the verbatim watermark weighting evidence | Representation critique: the dominant divergence class is kind-compatible and invisible to misfits (decisions 22, 45) |
| Database-reuse divergence as a flip site | Within-run evidence only, and stored-entry staleness is never ND evidence | Lifecycle critique: reuse divergence is a code-change signal (decision 9) |
| Persisted ND status plus a zero-divergence clearing heuristic | Only the representation persists, and the v2 entry is self-identifying | Lifecycle critique: inert in CI, and clearing flaps for passing-but-ND tests (decision 8) |
| "No stamping needed" once reporting is unified | Capture at confirmation: the engine stamps report-candidate executions | Lifecycle critique: capture decisions precede the body, so an unreproduced failure would report no values (decision 10) |
...
| Rollback to the last checkpoint on collapse | The monotone anchor, with no checkpoint/rollback at all | Statistics critique: geometric p-decay under re-baselining, and both rollback rules were then measured and dropped in experiment 001's follow-up (decisions 17, 19) |
| Unguarded single-run accepts inside a loop-until-K-dry fixpoint | The gauntlet (single-run rejects, every accept pays a sequential test) plus confirmed-dry stopping | Shrink-loop critique: accepts are sticky and burn monotone budgets (decision 7), with confirmed-dry measured later (decision 18) |
...
| Restore the data tree for ND runs, with conclusions as outcome distributions | Cut at v1 to detection plus novel prefixes, then deleted outright in phase 15 for the execution cache | Representation critique (distributions have no consumer), then experiment 010 and decision 60 (see [the seam plan](seam-plan.md)) |
...
cache that detects verdict flips, which the tree never could.
