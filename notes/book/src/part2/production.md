...
shrinking, NondetStash capture. The options were reusing value 3 with silently
changed semantics, appending a v2 status, or collapsing to plain `FAILED` plus a
per-failure caveat accessor. Closed as the third (decision 27): since decision 8
makes ND-ness a property of the entry rather than the run, the caveat is
...
heuristic. Closed as the heuristic (decision 28): boost runs only when the
...
tolerance. Closed as disabled (decision 29), with phase 7 assigned to measure
...
**G4, the strictness surface** (blocked phase 3). Confirmed as proposed
...
The largest semantic change: ND handling became detection-driven instead of
settings-gated. Mode entry needed no new detector, since the choice-tree
comparison in `record_run` already surfaced structural divergence and outcome
...
split, every quantitative runner behaviour silently changes meaning.
...
and blob, 005B's 139/139 standard as an automated test. It landed as b327fd1f.
...
unconfirmed-environmental (decision 3) — plus a static caveat for
concurrent-machine runs. Unconfirmed failures report with a clean origin and the
...
What landed (7a4fd194): concurrent-machine runs flow through the same pipeline
...
fresh case could fail for an unrelated reason — taking reproduction to 30/30 and
completing decision 25's one-replay-primitive rule (decision 33).
...
exit gate: the branch is the artefact decision 26 describes.
