...
persistence, blobs, and the verify pass (and with it the Flaky check, so the check that would
have detected nondeterminism was disabled by it). The first case creating a concurrent machine
was sacrificed as INVALID so later cases could be stamped before running. Reporting came from
a single-slot `NondetStash` that collapsed distinct bugs. Capture-at-confirmation (decision 10)
dissolved the constraint that forced the sacrifice, and phase 7 (7a4fd194) deleted the regime:
...
It was replaced by the timeline pool (decision 5): a flat failing incumbent plus a bounded pool of
...
structure) is invisible to both triggers. It was replaced by structural comparison signals, which
...
means a code change, so this mis-stamps deterministic tests routinely. It was replaced by
within-run evidence only (decision 9).

**Persisted ND status and rate estimates.** These were inert in CI (the database is disabled by
default), and the clear-on-zero-divergence heuristic flaps on tests that pass but are still ND.
They were replaced by persisting only the representation: v2 entries are self-identifying and
every run stands alone (decision 8).
...
the body runs, so an unreproduced failure would print a report with no values. It was replaced
by capture-at-confirmation (decision 10): the engine stamps replays capture-enabled. The stamp
...
deterministic 70% of the time, after which essentially all true reductions are rejected. The
streak classes were replaced by Wilson lower confidence bounds over one evidence ledger.
...
reads fine. It was replaced by the monotone anchor. Experiment 001's follow-up then dropped
checkpointing entirely (decision 17: rollback-on-uncertainty fires constantly on stable
landscapes at 2.3x cost, and rollback-on-proof never fires) and rejected anchor decay, which
makes stopping incoherent against a falling threshold (decision 19).
...
`BinSearchDown`'s low-probe state teleports the incumbent. It was replaced by charging accepts,
not rejects (decision 7): rejects stay single-run and retryable, and every accept pays the
...
Boost by Optimiser hill-climbing also died (three verified contradictions, and experiment
006A's holdout-gated successive-halving race replaced it), as did outcome distributions in the
tree. The statistics critique's lasting product was methodological: build a pure-simulation
calibration harness before touching the engine. The numbered experiment series grew out of
that instruction.
...
proposal skips the fast reject and drives its ledger evidence to a bound decision. The cost is
the same as three dry sweeps, the recoverable missed-reduction rate is half, and stopping
carries a certificate rather than a count. The final review later turned the stall guard off during
...
anchor, is why this was a recalibration rather than a redesign.

**The boost floor was in the wrong units.** Decision 28's 0.5 was written for the old
estimator, and against honest 20-run anchors it triggered on 59% of true-0.7 incumbents.
...
rescue rates had been measured at a 10-splice cap. The cap was corrected to 10 (decision 52).
...
rather than caching), but the question of what the tree buys had been asked, and experiment 010
...
tree had always silently overwritten, and experiment 011's baseline showed
the tree's own detection channel, the kind mismatch, firing zero times in 600 trials.
"Restore if feasible" ended in removal.
...
incumbent towards the minimal-bug floor before any detector fired, and a flip at shrink-verify
...
v1 blobs reproducing at 13%. The resolution (decisions 64-67, covered in
[the seam plan](seam-plan.md)) does not detect earlier so much as make pre-flip actions
...
**The backtrack walk.** The plan was a linear newest-first walk over the history. Review showed
it burns the scan budget on the degraded tail and re-runs the exact failure it exists to fix:
the bar admits a degraded recent entry and the anchor ratifies it (cc8a0c1f). It was
replaced by a geometric boundary scan with binary refinement, biased old under uncertainty.
Decision 2 makes the old bias safe (decision 66).

**History eviction.** The plan was a bounded ring with evict-oldest. Review showed evict-oldest
deletes the reproduction boundary exactly when shrinking went nondeterministic early, and the
memory the bound defended against had left with the tree (4b997a54). It was replaced by
...
continuation, held 99% on the same episodes: the fragility was in alignment rather than
example quality. The single shot was replaced by `V1_BLOB_REPLAYS = 4` budgeted attempts under
the standard continuation budget (decision 59).
...
produced it. Each instance was fixed the same way, by moving the accounting to a validated
event.
...
rather than the stopping rule (decision 54): a bar batch of four straight fails would otherwise
...
asserting the degenerate case as intended, and the caveat-only report with no values was pinned
...
caught by the suite, and both needed a harness running the machinery at a scale the tests do
not reach.
