...
Gate G20 was the one finding the remediation plan could not close in place: the
deterministic-to-ND seam. Raised on 2026-09-03 by phase 12's in-engine spot check
...
the target-regime landscape (L4b) and 15% on L4 — reported failures that were unshrunk,
unconfirmed, and unpersisted.

The mechanism named in the gate: production enters ND handling lazily, and a run that
...
question: what actually drives the seam, and is it just the data tree? No: the tree was
net protective on the seam workloads, supplying one of the run's two behavioural
...
2. **A late flip meets spent budgets.** Decision 23 priced the bar's 45% per-attempt
   ...
   with zero re-hunt budget: the 49% caveat-only rate.
3. **The never-flip corner.** Episodes with no detection event persisted v1 state whose
   ...
   was single-attempt, zero-continuation replay semantics, not example quality.
...
and silently overwrote a differently-concluding leaf, and detection power vanishes as
p → 1. Continuity costing found two cliffs. Trust: pre-flip rules act irreversibly on
n = 1, though most pre-flip protections would be free to run unconditionally. Cost:
post-flip, every accept pays ~20 physical runs, the reason a mode split exists at all.
"`nd_active` swaps roughly seven behaviors at once with nothing in between."

It then priced eight graded-middle options: (1) a bounded history of displaced
incumbents, consulted on flip; (2) provisional pre-flip pool capture; (3)
verify-as-bar-run-1, where the already-paid shrink-entry verify becomes the batch's first
evidence instead of a discarded boolean; (4) trusted admission from k ≥ 2 pre-flip
sightings; (5) a bounded re-hunt on a late flip; (6) v1 blob continuation and retry;
(7) provisional persistence from caveat-only runs; (8) routing measurement replays
through the tree as evidence conditional on determinism. Its own summary was that options
1–3 plus 6 close most of the measured loss at zero deterministic-run cost.
...
| --- | --- | --- |
| serving | 6.5x fewer bodies on the non-stateful shrink (200 vs 1308; 85% of shrink probes served), but inverted on the stateful shrink (10% serve rate, 9% more bodies, 39% more wall); serve count ≈ no-tree duplicate count (1146 vs 1134), so serves are almost all exact repeats | replace with a flat fingerprint(choices) → outcome cache |
| novel prefix | eliminates duplicates, which matters only on small spaces; no cell showed a discovery or shrink-quality difference | drop |
| exhaustion | decisive on tiny and filtered spaces (4 vs 1000 executions; the filtered cell stops at the 334 valid cases that exist instead of 3077 executions); inert elsewhere; never changed a verdict | replace with a duplicate-counter stop |
| recording | pure cost where the other roles are inert: 40–80% wall overhead on passing cells with zero counter movement | drop with the tree; re-home the mismatch check on the cache |
...
flip, exactly the outcome-ND evidence the tree silently swallowed.
...
- The backtrack walk became a geometric boundary scan (cc8a0c1f): a linear newest-first
  ...
  the loss. The scan instead probes at geometric offsets and binary-refines toward the
...
- History became keep-everything (4b997a54). Evict-oldest deletes the boundary exactly
  ...
  left with the tree. The recorded fallback is middle decimation, never end-eviction.
...
- L4b's 49% caveat-only was 43 bar power misses and 6 correct fluke rejections —
  mechanism 2 was 88% of the loss. L4's 15% was 15/15 correct fluke rejections: the bar
  working as designed on p = 0.02 noise.
...
Both cost guards held: passing-body parity exact at 50 executions on the pinned seed, and
the shrink-heavy body at 1510 executions against the 1661 bound, realising 010's serve
win. Two regressions were priced, not fixed: chain-only recursive depth spread
...
- **History** (decision 65, gate G24): `record_run`'s interesting arm appends every
  ...
  shrink accepts alike, deduplicated by serialized nodes, dropped on confirmation or run
  end. The hook is the arm, not `update_interesting`: after a fluke displaces the
...
aligned-outcome miss aborts as flaky, an as-built deviation); first-check replays run
stamped, amending decision 49; and decision 51's statistics line counts pre-flip check
...
- **D2 deterministic finals 68/100 against the 100/100 letter.** Thirty of the 80 flipped
  ...
  example with a v2 blob instead of the prettier deterministic core — priced honesty,
  but a letter miss as written.
...
the post-seam engine, and every criterion passed: never-flip 0/200 in every ND cell
(baseline 23/200 on clone at p = 0.9), with every flip landing at the first-interesting check; blob
reproduction 200/200 on both bodies at p = 0.9 (baselines 180 and 191), all blobs v2; the
p ≤ 0.3 cells held 009a's ≥ 98% reuse and blob rates; and the deterministic control paid
exactly 4 measurement replays per episode — the first check is a deterministic run's
whole ND cost.
...
decision 66's bar-gated backtrack — unbounded and scanned, not a newest-first ring, per
DRM. Option 2, provisional pool capture, is the same history, since the backtrack pools
...
1. **Pre-flip single-run trust inside a checked origin's shrink**, priced by 011's L1
   letters: final-p p50 0.74 rather than the 0.82 envelope, execs 1.54x against the 1.5x
   letter. An origin that passes an honest first check still shrinks on single-run trust
...
2. **The never-flip share that passes an honest check**, priced by 012 at 0/200 episodes
   ...
   check's escape rate (p·s)⁴ is small, not zero, and what escapes now carries a v1 blob
...
changelogs, and ran the full gate list green. Commit 43b2eb35 landed experiment 012,
decision 67, and the sweep — the branch's last phase commit before the [review fixes](../part1/status.md).
