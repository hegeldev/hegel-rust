...
a status of confirmed (code-verified) or plausible (sceptics split —
S7, L2, R5).

The verdict: "the architecture holds, but the statistics don't compose." Each constant
...
- **S1 (high).** The gauntlet — the mechanism built to kill unguarded single-run
  ...
  20-run confirmation batches, a stronger mechanism. S4: `GAUNTLET_FLOOR = 0.05` had
...
- **S3, M2.** `BOOST_RELIABILITY_FLOOR = 0.5` gated on an LCB whose confirmation-time
  ceiling was 0.5101, so boost was always-on in practice, and `REPRODUCE_SPLICES = 6`
...
- **Reporting and surface (R2, gates G11/G16/G17).** Generation-phase executions are
  stamped for capture once ND handling is active, so the never-reproduced report
  carries the discovering case's draw lines (decision 49). The ABI stamp was renamed
...
| --- | --- | --- | --- |
| `GAUNTLET_MIN_FAILS` | none (1 fail could accept) | 4 | Accept-rule ladder on L4b (bug kept / cost vs shipped): shipped 51%, m2 73%/1.37×, m3 97%/2.60×, m4 100%/3.01×. m4 costs 1.00× on 003's landscapes — its whole cost and whole value live in the target regime. m3 ruled out: no floor passes both floor criteria at m3 |
| `GAUNTLET_FLOOR` | 0.05, underived | 0.05, derived | 0.05 < LCB(4/30) = 0.0531, the min-fails acceptance boundary at the 30-run cap, so the floor costs zero power at m4 (at 0.08 power halves). False accept 4.0e-4 unconditional per proposal — resolves S4 |
...
Two deliberate retentions: the recruiting run stays counted (exclusion's 7× DP
...
out at 0.28–0.44 with exactly zero mass at weight 0, while the pre-decision-45
weighting, recomputed offline on the same replay pairs, put 78–97% of misses at
exactly zero — the old estimator had been running 008's w = 0 breakage column in
...
adopted mid-pass candidate — fixed with a bounds guard, the one production change of
...
The plan's conditional experiments never ran — a splice-budget 010 (resolved without
running by decision 52), a stamp-overhead 011, and a trusted-anchor 012 — which is why
...
the escalation signal stayed quiet (anchors 0.108/0.131 at p = 0.1), false accepts sat
...
place — the largest being flip-source attribution, persistence and reporting scoped to
...
with its pinning test, a decision entry, or a recorded deliberate retention (z = 1.96,
the asymmetric miss weighting, `FINAL_REPLAY_FRESH` chosen not derived —
decisions 53/54), with none untraceable. The full gate run came back green on
...
place rather than silently repaired (see [where the plan stands](../part1/status.md)).
...
| G12 | Trusted-shrink anchor | Evidence-batch LCB; trusted origins exempt from the bar's verdict (decision 47) |
...
could not close within its own frame: not a constant to recalibrate but a structural
property of the quiet-flip lifecycle, priced at a 49% caveat-only rate in the target
regime. It was carried through the phase-13 audit as open, and resolved the next day
