...
G5–G20) fixed the 41-finding as-built register and recalibrated the statistics. The seam plan
(phases 14–17, gates G21–G26) resolved G20, the deterministic-to-ND seam. The last gate closed
...
asymmetric miss weighting, `FINAL_REPLAY_FRESH` chosen not derived) are decisions 53/54 — and
carried exactly one item open: G20, raised by phase 12's in-engine spot check. The seam plan
closed it. The phase-17 closing sweep's full gate run (check, c-test, check-docs, miri,
...
| --- | --- | --- |
| G1 | ABI shape for ND failures | FAILED plus `hegel_failure_caveat`; status 3 retired and reserved (decisions 27/43) |
...
| G16 | Capture-predicate rename | `hegel_test_case_should_capture`, no shim — a compile-time migration signal (decision 50) |
...
`notes/evaluation.md` is the standing audit of the decision log: every entry 1–33 with where
it lives in the implementation, written 2026-09-03 against the phase-7 tree as production
phase 8's exit, then re-verdicted in place as the remediation and seam phases landed, so
its rows now cite decisions through 66. The verdicts fall into four shapes.
...
removal, the "never serve cached conclusions under ND" invariant surviving verbatim on the
...
- 0f504c52 — ND origin lifecycle: a write-only Unconfirmed rejection counter removed, the
  actual Unconfirmed-to-Confirmed admission paths named in the state-machine docs, and
  lifecycle tests pinned to what they claim.
- 48894dc4 — frontend, ABI docs, and notes: capture/blob documentation aligned across both
  crates with the header regenerated, two report-site `expect`s replaced with internal-error
  framing, a C-ABI test replaying an ND blob through `hegel_run_start_blob`, and notes
  corrections, including the C2/C9/C10 annotations and the audit count 40 → 41.

Two of these reverse remediation-era choices: the demote-not-delete supersession, and the
decode bound's interaction with the encoder's own output.
...
fake (0/100 flips, free displacement): kept 37, gained 13, lost 0, never-flipped 30 — a
confirmed p = 0.7 example with a v2 blob where free displacement would have found a smaller,
deterministic core.
...
slow bodies `MAX_SHRINKING_SECONDS` truncates instead. Escalated with no fix chosen — every
candidate fix trades against decision 2 — and recorded outside G20's loss accounting
(decision 67).
...
- `replay_aligned` under ND stays as accepted re-shrinking every run for structurally-ND
  bodies (005B priced it); revisit if slow real bodies bite.
...
(the annotations landed in 48894dc4) rather than silently repaired. The register they point at
was never located in the notes.
