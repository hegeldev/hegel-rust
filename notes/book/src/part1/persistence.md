...
(decision 8) — every run recomputes its estimates, every run stands alone, and
CI with the database disabled loses nothing but reuse. Two artefacts share the
...
serialised choice sequence, and a v2 entry is `NdReproState`, the replay state a
...
Failures are silent no-ops — a non-writable database must never abort an
...
## The v1 entry: serialised choices
...
- per choice, a 1-byte tag: 0 Integer, 1 Boolean, 2 Float, 3 Bytes, 4 String, 5
  Clone
- Integer: a 1-byte width sub-tag — always 10 (BigInt) on write, a 4-byte LE
  length then that many two's-complement LE bytes; legacy sub-tags 0–9 are
...
- Clone: the cloned stream's child values in the same count-then-entries layout,
  recursively — values only; spans and kinds are recreated on replay (decision
  32)
...
the stored failing timelines with the incumbent first; `entropy`, a u64 seed for
the fresh draws a single replay may need past a divergence; and `extension`, the
fresh-draw budget beyond the incumbent's flattened length. `encode_nd_state`
writes:
...
the FNV-1a hash of the concatenated serialised timelines, so identical state
...
A blob is `base64(prefix_byte ++ payload)`. The prefix byte selects the payload:
0 (`PREFIX_RAW`) raw `serialize_choices` bytes, 1 (`PREFIX_ZLIB`) their zlib
compression, 2 (`PREFIX_ND_RAW`) raw `encode_nd_state` bytes, 3
...
(the `reuse_replays` flag exempts them from the measurement no-persist rule).
...
strictness is Error — state only an ND run writes (see
[detection](detection.md)). An undecodable entry is deleted from both keys. A v1
entry under a still-deterministic run replays exactly once; a v2 entry, or any
entry once the run has flipped, replays through `nd_reproduce` with the reuse
budget split per timeline plus `REPRODUCE_SPLICES` splices, stamped for capture.
...
the first failure, so a live bug costs about 1/p replays — the budget is spent
in full only on stale entries. A flat count of 10 would miss a rate-0.1 bug 35%
of the time, which is why the budget is derived rather than fixed (decision 11).
...
from both keys immediately — a single exact replay is conclusive. An ND entry's
strike is statistical: a miss after the full budget demotes a primary entry to
the secondary key (strike one) and deletes a secondary entry (strike two).

A reproduction from a stored entry marks the origin trusted with the reuse
evidence and exempts it from the first-interesting check — the entry already
replayed it (see [the lifecycle](lifecycle.md) for the Trusted state). When the
stored primary incumbent reproduces node-for-node, `replay_aligned` holds and
the shrink phase is skipped entirely.
...
`build_report` uses, so exactly the reported-with-blob origins persist. On a
deterministic run they are each origin's serialised choices. All new entries are
...
per key (five times the reuse phase's default secondary sampling ceiling), with
the shortlex-largest entries beyond the cap deleted. The cap is a resource bound
...
`REPRODUCE_SPLICES = 10` splices (decision 52), but zero fresh generations — a
fresh case could fail for a reason unrelated to the blob (decision 33). A
reproduction trusts the origin with the accumulated evidence and reports a
...
header steers callers toward `hegel_run_start_blob` for ND blobs.
