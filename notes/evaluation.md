...
| 6 | Data tree never serves under ND | Closed by decision 60: the tree is removed; the invariant survives verbatim on the execution cache — the flip flushes it, and nothing is served or recorded while `nd_active` |
...
| 9 | Detection uses within-run evidence only | Implemented: a stored entry that misses is demoted/deleted (`test_runner.rs` reuse loop), never a flip source; flips come from cache verdict mismatches, the first-interesting check and the verify/final-replay outcome checks (decisions 60/64); declared concurrency is no longer a flip source (decision 70). Decision 62 rejected the planned reuse-comparison channel on exactly this ground |
| 10 | Capture at confirmation | Implemented as the stamp contract: `capture_replays` around confirmation batches, DB-reuse replays, the final replay, first-check replays (decision 64), and blob replays; `hegel_test_case_should_capture` tells the client to capture. The sacrificed first case and `NondetStash` are gone |
...
| 17 | No checkpoint/rollback in the shrink loop | Implemented by absence; capture-at-confirmation handles the pinning hazard at source. Title clause amended by decision 66: the history backtrack is detection-triggered, bar-gated recovery of recorded state, not 17's rejected statistically-triggered rollback |
| 18 | Confirmed-dry stopping | Implemented: the shrink probe's sweep modes (`SweepMode`, `set_sweep_mode`) run one confirmation sweep after a dry sweep. Cost amended by decision 72: an escalated failure minimum makes "accepted nothing" easier to obtain, so budget-exhausted floor-threshold shrinks stop earlier and miss recoverable reductions |
...
| 20 | Raw interesting never displaces an occupied origin | Implemented: `update_interesting` fills vacant origins only; displacement goes through validated accepts. Annotated by decisions 65/66: pre-flip sightings are all kept in the origin history, a restore re-enters through the full bar, and the reuse phase's replays are the one measurement path allowed to displace |
| 21 | Discovery confirmation is a prerequisite | Implemented: `nd_discovery_sweep` after each generation step; unconfirmed origins are dropped and generation keeps hunting. Extended by decision 64: the first-interesting check now runs ahead of the sweep on every discovered origin, so the principle covers deterministic runs too |
| 22 | Pool cap 5-10, first-fit, small continuation budget, divergence weights evidence | Implemented: `POOL_CAP = 10`, `continuation_budget(len) = len + max(4, len/8)`; the weighting clause is superseded — misses count in full (decision 71) |
| 23 | Discovery bar: 10 gate / 40 cap / accept on 4th failure | Implemented: `GATE_RUNS`, `CONFIRM_CAP`, `CONFIRM_MIN_FAILS` in `nd/mod.rs`, exercised by `nd_evidence_batch`. Annotated by decisions 64/66: a first-check miss seeds the batch, and a backtracked candidate faces the same bar on the origin's `BACKTRACK_BAR_ATTEMPTS` budget. Amended by decision 72: batches spend `BAR_ATTEMPTS_PER_RUN = 5` per origin per run, so within-run recycling is capped and sub-target bugs lean on cross-run recycling |
...
| 25 | Replay order: first-fit, splices, fresh; boost off outside rescue | Implemented: `nd_reproduce` (splices = 10 — decision 52 corrected a transcription of 006B's cap; fresh only where the caller allows), `nd_boost` behind the reliability floor (0.30 since decision 56) |
...
| 29 | G3: data tree disabled under ND | Closed by decision 60 (see 6): removal, not disablement. The kind-drift detection the tree carried is decision 62's within-run ledger, `error` strictness only |
| 30 | G4: strictness surface | Implemented (see 1); `error` reproduces the old abort diagnostics verbatim (`flaky_diagnostic`, pinned by tests) and now aborts earlier: the cache's verdict mismatch and the first check catch at discovery what the old engine caught at shrink verify or never, and a structural check miss carries a position-naming diagnostic (decisions 60/64) |
...
- The bytes-increment shrinker hole (decision 63) waits on a probe-based increment variant:
  execute the bumped proposal and accept on the realized early exit.
