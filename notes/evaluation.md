...
| 23 | Discovery bar: 10 gate / 40 cap / accept on 4th failure | Implemented: `GATE_RUNS`, `CONFIRM_CAP`, `CONFIRM_MIN_FAILS` in `nd/mod.rs`, exercised by `nd_evidence_batch`. Annotated by decisions 64/66: a first-check miss seeds the batch, and a backtracked candidate faces the same bar up to `BACKTRACK_BAR_ATTEMPTS` times |
