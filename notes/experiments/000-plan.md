...
| 009b | Composed-rules re-verification (`/experiments/watermark`, `composed` subcommand) | Do 009a's operating points hold under the phase-12 composed rules; composed-engine G10 recheck | done — escalation signal quiet (anchors 0.108/0.131 at p = 0.1), false accept at or under the DP's 4.0e-4, proof-rejects 10-13 at informative anchors, low-anchor fluke rejects ride the 30-cap by arithmetic, reuse/blob >= 98% at p <= 0.3; see `009b-composed-rules/notes.md` |
| 010 | Data-tree value on production main (local branch `claude/experiment-010-tree-value` off main@770970b8, commit 19a47940 — the harness patches main's engine, so it is not frozen under `/experiments` here) | What do the tree's four roles (recording, serving, novel prefix, exhaustion) buy on realistic workloads (G20 option d) | done — recording/novel-prefix/exhaustion buy nothing measurable on large-space workloads (recording alone 40-80% wall overhead at cheap-body prices); serving is a 6.5x execution win on non-stateful shrinking (85% of probes served, ~all exact repeats) but net-negative on the stateful shrink; exhaustion decisive only on tiny/filtered spaces; flat fingerprint cache + duplicate-counter stop recover both wins without the tree; see `010-tree-value/notes.md`. (The splice comparison G18 once sketched as "experiment 010" was resolved by decision 52 without running; the number is reused) |
| 011 | Instrumented seam spot check (`/experiments/gauntlet-calibration`, amended with `__bench` event columns) | Decompose the seam losses (flip site/time, caveat-only decomposition) on the current engine, then accept or reject the seam-plan engine against the letters in `seam-plan.md` | done (2026-09-04; baseline at ad3ff0c1, comparison at 532be034): caveat-only 49%/15% -> 0/0, L1 final-p median 0.34 -> 0.74; two letter misses escalated to DRM (L1 execs 1.54x vs the 1.5x letter; D2 det finals 68/100 vs 100/100, decomposed as pre-core flips under the post-flip displacement freeze); see `011-seam-spot-check/notes.md`. (Remediation-plan's conditional 011, stamp overhead, never ran; the number is reused) |
| 012 | Detection-escape recheck (new frozen crate cloning `experiments/watermark`'s episode protocol) | Do the first-interesting check and the v1 continuation fix close 009a's never-flip corner (23/200 episodes, 13% v1 blob reproduction at p = 0.9) | done — all criteria pass: never-flip 0/200 (was 23/200), blob reproduction 200/200 both bodies at p = 0.9 (was 180/191), >= 98% at p <= 0.3, control pays exactly the k = 4 check; found the gauntlet cost lottery at p above the high-water (~1M replays/episode, escalated); see `012-detection-escape/notes.md` |
