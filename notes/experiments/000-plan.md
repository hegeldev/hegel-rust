...
| 011 | Instrumented seam spot check (`/experiments/gauntlet-calibration`, amended with `__bench` event columns) | Decompose the seam losses (flip site/time, caveat-only decomposition) on the current engine, then accept or reject the seam-plan engine against the letters in `seam-plan.md` | baseline done (2026-09-04, engine ad3ff0c1): L4b's 49% caveat-only = 43 power misses + 6 correct fluke rejections; L1 incumbent-p at flip 0.26 at every percentile (the loss is entirely pre-flip); tree kind-mismatch fired 0/600 trials; comparison half runs in phase 17; see `011-seam-spot-check/notes.md`. (Remediation-plan's conditional 011, stamp overhead, never ran; the number is reused) |
| 012 | Detection-escape recheck (new frozen crate cloning `experiments/watermark`'s episode protocol) | Do the first-interesting check and the v1 continuation fix close 009a's never-flip corner (23/200 episodes, 13% v1 blob reproduction at p = 0.9) | spec'd — runs in phase 17; criteria in `seam-plan.md`. (Remediation-plan's conditional 012, trusted anchor, never ran; the number is reused) |
