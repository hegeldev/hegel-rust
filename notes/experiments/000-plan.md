...
| 011 | Instrumented seam spot check (`/experiments/gauntlet-calibration`, amended with `__bench` event columns) | Decompose the seam losses (flip site/time, caveat-only decomposition) on the current engine, then accept or reject the seam-plan engine against the letters in `seam-plan.md` | spec'd — baseline half in phase 14, comparison half in phase 17. (Remediation-plan's conditional 011, stamp overhead, never ran; the number is reused) |
