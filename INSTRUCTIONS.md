...
- [x] **A20.** Span-aware shrinker passes are entirely missing: `pass_to_descendant`, `try_trivial_spans`, `reorder_spans`, `reduce_each_alternative`, `node_program`. Port each. **Split into A20a–A20e below per the audit's "probably split" hint; this parent is closed when all sub-items are.**
  ...
  - [ ] **A20d.** `reduce_each_alternative` (`shrinker.py:644`). Not yet ported. Note this is fired by `initial_coarse_reduction`, not by the standard shrink-passes list; the wiring will be different.
  - [ ] **A20e.** `node_program` (`shrinker.py:1273`). Not yet ported. Five sizes registered upstream (`X`, `XX`, `XXX`, `XXXX`, `XXXXX`); A20e covers all of them.
