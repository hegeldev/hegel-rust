...
/// active flags inherited from inline modifiers and call-site arguments.
#[derive(Clone, Debug)]
...
/// token `\X` produced by the tokenizer when it sees a backslash.
...
/// Port of `_parser.fix_flags`, str-pattern path only — the only entry
/// point ([`parse_pattern`]) parses str patterns.
