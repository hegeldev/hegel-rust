...
fn new_concatenates_offsets() {
    let iv = IntervalSet::new(vec![(0, 2), (10, 12)]).unwrap();
    assert_eq!(iv.len(), 6);
...
fn get_out_of_range_returns_none() {
    let iv = IntervalSet::new(vec![(0, 2)]).unwrap();
...
fn get_negative_index_resolves_from_end() {
    let iv = IntervalSet::new(vec![(5, 9)]).unwrap();
...
fn index_returns_position_when_present() {
    let iv = IntervalSet::new(vec![(10, 12), (20, 22)]).unwrap();
...
fn index_returns_none_between_intervals() {
    let iv = IntervalSet::new(vec![(0, 5), (10, 15)]).unwrap();
    assert!(iv.index(7).is_none());
}

#[test]
fn index_returns_none_past_all_intervals() {
    let iv = IntervalSet::new(vec![(0, 5)]).unwrap();
    assert!(iv.index(100).is_none());
}

#[test]
fn union_with_empty_other_returns_self() {
    let a = IntervalSet::new(vec![(0, 5)]).unwrap();
    let b = IntervalSet::new(Vec::new()).unwrap();
...
fn union_with_empty_self_returns_other() {
    let a = IntervalSet::new(Vec::new()).unwrap();
    let b = IntervalSet::new(vec![(0, 5)]).unwrap();
    let u = a.union(&b).unwrap();
    assert_eq!(u, b);
}
...
fn union_merges_overlapping_intervals() {
    let a = IntervalSet::new(vec![(0, 5), (20, 25)]).unwrap();
    let b = IntervalSet::new(vec![(3, 10), (24, 30)]).unwrap();
    let u = a.union(&b).unwrap();
    assert_eq!(u, IntervalSet::new(vec![(0, 10), (20, 30)]).unwrap());
}

#[test]
fn union_merges_adjacent_intervals() {
    let a = IntervalSet::new(vec![(0, 5)]).unwrap();
    let b = IntervalSet::new(vec![(6, 10)]).unwrap();
    let u = a.union(&b).unwrap();
    assert_eq!(u, IntervalSet::new(vec![(0, 10)]).unwrap());
}

#[test]
fn difference_advances_past_smaller_other_interval() {
    let a = IntervalSet::new(vec![(20, 30)]).unwrap();
    let b = IntervalSet::new(vec![(0, 5)]).unwrap();
...
fn difference_carves_middle() {
    let a = IntervalSet::new(vec![(0, 10)]).unwrap();
    let b = IntervalSet::new(vec![(4, 6)]).unwrap();
    let d = a.difference(&b).unwrap();
    assert_eq!(d, IntervalSet::new(vec![(0, 3), (7, 10)]).unwrap());
}

#[test]
fn intersection_advances_past_smaller_self_interval() {
    let a = IntervalSet::new(vec![(0, 5)]).unwrap();
    let b = IntervalSet::new(vec![(20, 30)]).unwrap();
    let r = a.intersection(&b).unwrap();
    assert_eq!(r, IntervalSet::new(Vec::new()).unwrap());
}

#[test]
fn intersection_overlapping_intervals() {
    let a = IntervalSet::new(vec![(0, 10), (20, 30)]).unwrap();
    let b = IntervalSet::new(vec![(5, 25)]).unwrap();
    let r = a.intersection(&b).unwrap();
    assert_eq!(r, IntervalSet::new(vec![(5, 10), (20, 25)]).unwrap());
}

#[test]
fn char_in_shrink_order_round_trips_via_index() {
    let iv = IntervalSet::new(vec![(0, 0xD7FF), (0xE000, 0x10FFFF)]).unwrap();
