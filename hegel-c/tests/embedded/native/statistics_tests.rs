...
fn stdtr_matches_scipy_oracle() {
    for &(df, t, expected) in oracle::STDTR_CASES {
        let got = stdtr(df, t).unwrap();
        assert_close(
            got,
            expected,
            1e-12,
            1e-15,
            &format!("stdtr(df={df}, t={t}).unwrap()"),
        );
    }
}
...
fn stdtrit_strict_matches_scipy_oracle_for_df_le_2() {
    for &(df, p, expected) in oracle::STDTRIT_STRICT_CASES {
        let got = stdtrit(df, p).unwrap();
        assert_close(
            got,
            expected,
            1e-15,
            0.0,
            &format!("stdtrit(df={df}, p={p}).unwrap()"),
        );
    }
}
...
fn stdtr_is_symmetric_around_zero() {
    for df in [1, 2, 3, 4, 10, 50] {
        for t in [0.1, 0.5, 1.0, 2.0, 5.0, 100.0] {
            let lhs = stdtr(df, t).unwrap();
            let rhs = 1.0 - stdtr(df, -t).unwrap();
...
fn stdtr_at_zero_is_one_half() {
    for df in [1, 2, 3, 4, 17, 100] {
        assert_eq!(stdtr(df, 0.0).unwrap(), 0.5);
    }
}
...
fn stdtrit_at_one_half_is_zero() {
    for df in [1, 2, 3, 4, 17, 100] {
        assert_eq!(stdtrit(df, 0.5).unwrap(), 0.0);
    }
}
...
fn stdtrit_inverts_stdtr() {
    for &(df, tol) in &[(1, 1e-12), (2, 1e-12), (3, 1e-7), (10, 1e-7)] {
        for t in [-5.0, -1.0, -0.1, 0.1, 1.0, 5.0] {
            let p = stdtr(df, t).unwrap();
            let back = stdtrit(df, p).unwrap();
...
#[test]
fn stdtr_errors_on_df_zero() {
    let msg = stdtr(0, 0.0).unwrap_err().to_string();
    assert!(msg.contains("df >= 1"), "{msg}");
}

#[test]
fn stdtrit_errors_on_df_zero() {
    let msg = stdtrit(0, 0.5).unwrap_err().to_string();
    assert!(msg.contains("df >= 1"), "{msg}");
}

#[test]
fn stdtrit_errors_on_p_out_of_range_low() {
    let msg = stdtrit(2, 0.0).unwrap_err().to_string();
    assert!(msg.contains("0 < p < 1"), "{msg}");
}

#[test]
fn stdtrit_errors_on_p_out_of_range_high() {
    let msg = stdtrit(2, 1.0).unwrap_err().to_string();
    assert!(msg.contains("0 < p < 1"), "{msg}");
}
...
fn uniform_inverse_cdf_spans_range() {
    ...
    assert_eq!(d.inverse_cdf(0.0).unwrap(), -256.0);
    assert_eq!(d.inverse_cdf(0.5).unwrap(), 0.0);
    assert_eq!(d.inverse_cdf(1.0).unwrap(), 256.0);
}

#[test]
fn uniform_cdf_linear() {
    let d = UniformDistribution::new(10.0);
    assert_eq!(d.cdf(-10.0).unwrap(), 0.0);
    assert_eq!(d.cdf(0.0).unwrap(), 0.5);
    assert_eq!(d.cdf(10.0).unwrap(), 1.0);
    assert_eq!(d.cdf(5.0).unwrap(), 0.75);
}
...
fn uniform_cdf_clamps_outside_support() {
    let d = UniformDistribution::new(10.0);
    assert_eq!(d.cdf(-100.0).unwrap(), 0.0);
    assert_eq!(d.cdf(100.0).unwrap(), 1.0);
}
...
fn piecewise_pdf_splits_at_switchover_and_matches_continuity() {
    ...
    let d = PiecewiseDistribution::new(inner, outer, 256.0).unwrap();
...
fn log_student_t_round_trip() {
    ...
    for x in [1.0, 10.0, 100.0, 1_000.0, 1_000_000.0] {
        ...
        let back = d.inverse_cdf(p).unwrap();
...
fn log_student_t_clamps_extreme_inverse_cdf_inputs() {
    let d = LogStudentTDistribution::new(13.0, 2);
    let lo = d.inverse_cdf(1e-300).unwrap();
    let hi = d.inverse_cdf(1.0 - 1e-15).unwrap();
...
fn piecewise_density_is_continuous_at_switchover() {
    ...
    let outer = LogStudentTDistribution::new(13.0, 2);
    let d = PiecewiseDistribution::new(inner, outer, 256.0).unwrap();
    let left_density = d.cdf(256.0).unwrap() - d.cdf(255.0).unwrap();
    let right_density = d.cdf(257.0).unwrap() - d.cdf(256.0).unwrap();
