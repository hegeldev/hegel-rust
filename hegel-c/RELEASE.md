...
- `hegel_new_state_machine` gains `group_names`/`num_groups`, `rule_groups` (group indices parallel to `rule_names`), and `concurrency` parameters. A sequential machine passes a single group, all-zero `rule_groups`, and concurrency 1.
- `hegel_state_machine_next_rule` gains a `thread_index` parameter and now hands out rules for one round at a time; the frontend must advance rounds with `hegel_state_machine_next_group`, even for sequential machines.
