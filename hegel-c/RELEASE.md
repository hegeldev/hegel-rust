...
This patch raises the default limit on the number of choices a single test case may make from 8,192 to 2^20 (1,048,576), and adds `hegel_settings_set_max_choices` to change or remove it. Suppressing the `TestCasesTooLarge` health check, or passing a `max_choices` of 0, makes test cases unbounded: a draw then never fails with `HEGEL_E_STOP_TEST` for running out of room. The `LargeInitialTestCase` health check now measures against the configured limit rather than the fixed buffer size.
