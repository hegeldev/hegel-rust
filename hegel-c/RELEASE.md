...
- The engine now runs every reported failure one final time before the run concludes, with the case stamped via `hegel_test_case_should_capture` (which also stamps confirmation batches, database-reuse replays, and generation cases under nondeterministic handling). Clients should capture stamped cases' output per origin and report each failure from the freshest capture instead of replaying its blob after the run.
