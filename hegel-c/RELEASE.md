...
The engine's locking now goes through that platform layer too, replacing `parking_lot` and the standard library's locks with a futex-backed mutex and a lock-free lazy initialiser. With those gone, libhegel registers no thread-local storage and no thread-exit destructors of its own, so a thread that used libhegel and outlives a `dlclose` of it no longer crashes at exit.
