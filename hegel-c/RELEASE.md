...
This changes the recorded choice sequence of pool draws, so previously saved database entries and reproduction blobs for tests using pools no longer reproduce their failures; stale database entries are discarded automatically, while reproduction blobs fail loudly and should be regenerated.
