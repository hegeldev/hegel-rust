    ...
    let print_match_arms: Vec<_> = variants
        ...
        .map(|(i, variant)| {
            ...
            match &variant.fields {
                ...
                _ => {
                    let field_name = variant_to_field[&variant.ident.to_string()];
                    quote! { #i => self.#field_name.do_draw_and_print(__tc, __printer) }
                }
            }
        })
        .collect();
...
fn generate_variant_generator(
    ...
    let (construction, print_construction, print_actions) = if construction_shape {
        ...
        let print_actions: Vec<_> = field_idents
            ...
            .map(|ident| {
                quote! {
                    let #ident = self.#ident.do_draw_and_print(__tc, __printer);
                }
            })
            .collect();
        (
        ...
        let print_actions: Vec<_> = print_idents
            ...
            .map(|(print_ident, field_ident)| {
                quote! {
                    let #print_ident = self.#field_ident.do_draw_and_print(__tc, __printer);
                }
            })
            .collect();
        (
