...
fn list_and_int() -> impl Generator<(Vec<i64>, i64)> {
    gs::vecs(gs::integers::<i64>()).flat_map(|v| {
        gs::integers::<i64>().map(move |i| (v.clone(), i))
    })
}
...
fn test_containment() {
    for n in [0i64, 1, 10, 100, 1000] {
        let (ls, i) = find_any(
            list_and_int(),
            move |(ls, i): &(Vec<i64>, i64)| *i >= n && ls.contains(i),
        );
        assert!(i >= n);
...
fn test_duplicate_containment() {
    let (ls, i) = find_any(
        list_and_int(),
        |(ls, i): &(Vec<i64>, i64)| ls.iter().filter(|&&x| x == *i).count() > 1,
    );
...
fn test_can_find_list_with_sum() {
    let result = find_any(
        ...
        |xs: &Vec<i64>| xs.iter().copied().fold(0i64, i64::saturating_add) >= 10,
    );
