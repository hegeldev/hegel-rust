...
//! up by one in the same move, and the passes that pair a deletion with a value change all
//! *lower*. The draw to raise is behind the deletable span and is not a gate, so
//! `try_shortening_via_increment` (which raises a gating draw to drop what it guards) does not
//! reach it. A human writes `([], 5)` too.
