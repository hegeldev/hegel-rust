...
fn test_sampled_from_references(tc: TestCase) {
    ...
    let value: &i32 = tc.draw(gs::sampled_from(refs));
...
fn test_sampled_from_str_references(tc: TestCase) {
    ...
    let value: &str = tc.draw(gs::sampled_from(strings.to_vec()));
...
fn test_tuple_of_references(tc: TestCase) {
    ...
    let (x, y): (&i32, &&str) = tc.draw(gs::tuples!(
        gs::sampled_from(x_refs),
        gs::sampled_from(y_refs),
    ));
...
fn test_optional_of_references(tc: TestCase) {
    ...
    let result: Option<&i32> = tc.draw(gs::optional(gs::sampled_from(refs)));
...
fn test_one_of_with_references(tc: TestCase) {
    ...
    let value: &i32 = tc.draw(hegel::one_of!(
        gs::sampled_from(small_refs),
        gs::sampled_from(big_refs),
    ));
...
fn test_vec_of_references(tc: TestCase) {
    ...
    let refs: Vec<&i32> = options.iter().collect();
    let result: Vec<&i32> = tc.draw(gs::vecs(gs::sampled_from(refs)).min_size(1).max_size(5));
...
fn test_map_over_references(tc: TestCase) {
    let values = [10, 20, 30];
    let refs: Vec<&i32> = values.iter().collect();
    let doubled: i32 = tc.draw(gs::sampled_from(refs).map(|r| r * 2));
...
fn test_tuple3_of_references(tc: TestCase) {
    ...
    let (x, y, z): (&i32, &&str, &bool) = tc.draw(gs::tuples!(
        gs::sampled_from(xr),
        gs::sampled_from(yr),
        gs::sampled_from(zr),
    ));
...
fn test_nested_optional_tuple_of_references(tc: TestCase) {
    ...
    let result: Option<(&&str, &u32)> = tc.draw(gs::optional(gs::tuples!(
        gs::sampled_from(name_refs),
        gs::sampled_from(age_refs),
    )));
...
fn test_vec_of_tuples_of_references(tc: TestCase) {
    ...
    let result: Vec<(&i32, &&str)> =
        tc.draw(gs::vecs(gs::tuples!(gs::sampled_from(kr), gs::sampled_from(vr),)).max_size(5));
    for (k, v) in &result {
...
fn test_one_of_mapped_references(tc: TestCase) {
    ...
    let description: String = tc.draw(hegel::one_of!(
        gs::sampled_from(pos_refs).map(|r| format!("positive: {}", r)),
        gs::sampled_from(neg_refs).map(|r| format!("negative: {}", r)),
    ));
...
fn test_deeply_nested_reference_composition(tc: TestCase) {
    ...
    let result: Vec<i32> = tc.draw(
        gs::vecs(
            gs::optional(gs::tuples!(gs::sampled_from(xr), gs::sampled_from(yr),)).map(|opt| {
                match opt {
                    Some((a, b)) => a + b,
                    None => 0,
                }
...
fn test_boxed_generator_with_local_lifetime(tc: TestCase) {
    ...
    let ix = gs::integers().min_value(0).max_value(2);
