...
fn is_hegel_file_recognizes_src_paths() {
    ...
    assert!(is_hegel_file("src/generators/deferred.rs"));
...
// that was never `set()` — as opposed to an explicit `hegel_internal_error!`
// raise, which unwinds as a typed payload), `run_test_case` must re-raise it
// immediately as a hegel internal error rather than shrinking it as a
// discovered counterexample. This exercises that path in-process (the
// subprocess tests in `tests/test_internal_errors.rs` pin the exact
// user-visible format).
